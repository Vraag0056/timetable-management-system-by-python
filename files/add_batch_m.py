...
def remove_data():
    ...
    for i in tree.selection():
        ...
        cur = conn.cursor()
        cur.execute(f"DELETE FROM batch_m WHERE batch_m_name = '{tree.item(i)['values'][0]}'")
        cur.execute("commit")
        tree.delete(i)
        update_treeview()
...
def update_treeview():
    ...
    for row in tree.get_children():
        tree.delete(row)
    cur = conn.cursor()
    cur.execute("SELECT * FROM batch_m")
    cur = list(cur)
    for row in cur:
...
def parse_data():
    ...
    cur = conn.cursor()
    cur.execute(f"insert into batch_m (batch_m_name) values ('{subname}')")
    cur.execute("commit")
    update_treeview()
...
root = Tk()
root.title('Add Batch - Management Department')
...
Label(root,text='Created By : Divyanshu Jain',bg="#fff").place(x=500,y=425)
root.mainloop()
