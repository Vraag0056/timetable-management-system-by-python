...
def ms_excel():
    ...
    new_path = "timetable_excel\\timetable_management.xlsx"
...
def update(var, j, x):
    ...
    cur = conn.cursor()
    cur.execute(
        f"UPDATE schedule_m SET day='',timing='',course='',subject='',faculty='',room='' WHERE  btn_no= '{var}'")
    cur.execute("commit")
    com = ''
    for key in range(0, 6):
        if ((var - key) % 30 == 0 or (var - key) == 0):
            com = combo[0][key].get()
            Label(root1, text="Course: " + combo[0][key].get(), fg='black', bg='white', borderwidth=3, width=25,
                  relief="sunken",
    ...
    for key in range(0, 6):
        if ((var - key - 6) % 30 == 0 or (var - key - 6) == 0):
            com = combo[1][key].get()
            Label(root1, text="Course: " + combo[1][key].get(), fg='black', bg='white', borderwidth=3, width=25,
                  relief="sunken",
                  font=('Consolas', 18, 'bold')).place(x=5, y=90)
    ...
    for key in range(0, 6):
        if ((var - key - 12) % 30 == 0 or (var - key - 12) == 0):
            com = combo[2][key].get()
            Label(root1, text="Course: " + combo[2][key].get(), fg='black', bg='white', borderwidth=3, width=25,
                  relief="sunken",
                  font=('Consolas', 18, 'bold')).place(x=5, y=90)
    ...
    for key in range(0, 6):
        if ((var - key - 18) % 30 == 0 or (var - key - 18) == 0):
            com = combo[3][key].get()
            Label(root1, text="Course: " + combo[3][key].get(), fg='black', bg='white', borderwidth=3, width=25,
                  relief="sunken",
                  font=('Consolas', 18, 'bold')).place(x=5, y=90)
    ...
    for key in range(0, 6):
        if ((var - key - 24) % 30 == 0 or (var - key - 24) == 0):
            com = combo[4][key].get()
            Label(root1, text="Course: " + combo[4][key].get(), fg='black', bg='white', borderwidth=3, width=25,
                  relief="sunken",
                  font=('Consolas', 18, 'bold')).place(x=5, y=90)
    ...
    if (x == 0 and j == 0): #lecture 1 ----- Monday
    ...
    if (x == 0 and j == 1):  #lecture 1 ----- Tuesday
    ...
    Button(root1, width=39, pady=7, text='Set', bg='#57a1f8', fg='white', border=0,
           command=lambda var=var, stu_c=combo1, q=q, t=t, fac_ini=combo2, room=room, j=j, x=x, root1=root1: change(var,
                                                                                                                    fac_ini,
                                                                                                                    stu_c,
                                                                                                                    room,
                                                                                                                    j,
...
def get_bat_com(i,j,k):
    ...
    for lq in range(i + k, j + k, 30):
        sch_f1 = conn.cursor()
        sch_f1.execute(f"SELECT course FROM schedule_m where btn_no={lq}")
        sch_f1 = list(sch_f1)
        if sch_f1[0][0] != '':
            b.pop(0)
                    ...
                    if(x==0 and j==0):
                        ...
                        if d[fac_ini.get()]>2:
                            ...
                            if answer:
                                cur = conn.cursor()
                                cur.execute(
                                    f"UPDATE schedule_m SET day='{days[j]}',timing='{time[x].get()}',course='{com}',subject='{sub_c.get()}',faculty='{fac_ini.get()}',room='{room.get()}' WHERE  btn_no= '{var}'")
                                cur.execute("commit")
...
root = Tk()
root.title('Set Timetable ---- Management Department')
...
batch = conn.cursor()
s = f"SELECT batch_m_name from batch_m"
batch.execute(s)
batch = list(batch)
...
sch = conn.cursor()
sch.execute("SELECT subject,faculty,room FROM schedule_m")
sch=list(sch)
op=0
update_init_no()
for k in range(0, 8):
